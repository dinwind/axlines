...
|-------------|---------|-------------|
| `main` | `main` | Default integration branch (PR base) |
| `origin` | `origin` | Remote name for fetch/push |
...
| `scripts/start-task-branch.ps1` | `scripts/start-task-branch.ps1` | Start task branch from latest trunk |
| `scripts/post-merge-cleanup.ps1` | `scripts/post-merge-cleanup.ps1` | **Mandatory** post-merge cleanup |
| `*(optional)*` | `scripts/push-and-pr.ps1` | Optional PR creation script |
...
| P5 | **Before merge**: integrate latest `origin/main` into the head branch; resolve conflicts; ensure mergeable |
...
| `origin/main` | PR **base** — integration truth |
| **PR head** | Tip of the remote feature branch; new pushes update head and may require **re-approve** |

**Recommended lifecycle:**

```text
co branch start fix/<topic>          # or scripts/start-task-branch.ps1 -Branch fix/<topic>
# ... develop, test ...
git push -u origin fix/<topic>
# open PR: head=fix/<topic>, base=main
# after merge (MANDATORY): co branch cleanup fix/<topic>
```

**Anti-patterns:**

- Opening PR #2, #3 on a branch that already merged to `main`
...
|-------------|----------|
| Merged code facts | **Git** (`main`, tags) |
...
4. `git log origin/main --oneline -5`
...
```text
1. co branch start fix/<kebab-topic>   # or scripts/start-task-branch.ps1 -Branch fix/<kebab-topic>
2. implement + {{TEST_PRE_PR}}
3. write journal.d fragment for completed items
4. git add <product paths only>
5. git commit
6. git fetch && git rebase origin/main   # or merge; resolve conflicts
7. git push -u origin fix/<kebab-topic>
8. open PR (*(optional)* or host UI)
9. before merge: §6 gates
10. after merge (MANDATORY): co branch cleanup fix/<kebab-topic>
    # or scripts/post-merge-cleanup.ps1 -Branch fix/<kebab-topic>
11. if needed: chore/agent-status for journal-flush
```
...
| 4 | Remaining branches: `git fetch` + rebase `origin/main`, then push |
...
For `status.md` / `version-state.toml` conflicts: take `main` as base, then apply the correct **track** content per §3 — never `merge=union` on status.
...
| Check | Method |
|-------|--------|
| Head contains latest trunk | `git fetch origin` then `git merge-base --is-ancestor origin/main HEAD` (exit 0) |
...
if ($LASTEXITCODE -ne 0) { Write-Error "Branch is behind main; integrate first." }
```
...
- [ ] **MANDATORY:** `co branch cleanup <head>` or `scripts/post-merge-cleanup.ps1 -Branch <head>` (remote head deleted)
- [ ] Local checkout on `main`, synced with `origin/main`
...
| Local trunk behind | Infrequent fetch | Use `origin/main` as truth during development |
