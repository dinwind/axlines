...
When `version-state.toml` is missing or still contains scaffold placeholders (`[DATE]`, `[path/to/...]`):

1. Run `co version audit` (or `co version audit --json` in CI/agents).
2. If status is `unconfigured` or `missing`, initialize with explicit operator inputs:

```bash
co version bootstrap --track primary --canonical <path/to/canonical-source> --version <x.y.z> --apply
```

3. Re-run `co version audit` until status is `configured` and `co lint --rule version-state` passes.
...
| Tool | Role |
|------|------|
| `.agent/scripts/verify-versions.py` (from bundled template) | Local Drift Guard: reads canonical source + `version-state.toml`; exit 1 on frozen/released drift; planning window allowed when `developing` and canonical == `current_release`. |
